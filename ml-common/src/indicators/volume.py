...
High-performance volume-based indicators для crypto trading analysis.
Implements Context7 enterprise patterns с Numba optimization.
...
def _fast_obv(prices: np.ndarray, volumes: np.ndarray) -> float:
    """Fast OBV calculation с Numba optimization"""
...
def _fast_vwap(prices: np.ndarray, volumes: np.ndarray, period: int) -> float:
    """Fast VWAP calculation с Numba optimization"""
...
def _fast_mfi(high: np.ndarray, low: np.ndarray, close: np.ndarray,
    ...
    """Fast MFI calculation с Numba optimization"""
    ...
    # Separate positive и negative money flows
    positive_flow = 0.0
    negative_flow = 0.0
...
def calculate_chaikin_money_flow(
    ...
    # Calculate money flow multiplier для each period
    ...
    # Calculate CMF для recent period
...
def calculate_volume_profile(
    ...
    Returns:
        VolumeProfileResult с profile data
    """
    if not all([high, low, close, volumes]) or len(high) < period:
    ...
    # Find Point of Control (POC) - price level с highest volume
    ...
    # Start from POC и expand outwards
...
@dataclass
class VolumeConfig:
    """Configuration для volume indicators"""
    vwap_period: int = 20
    mfi_period: int = 14
    cmf_period: int = 20
...
class VolumeIndicators:
    ...
    vectorized operations и optional Numba acceleration.
    """
    ...
    def calculate_all(
        ...
        Returns:
            Dictionary с all volume indicator values
        """
        results = {}
