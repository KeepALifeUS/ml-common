"""
Test Suite для ML Common Package

Comprehensive test suite implementing Context7 testing patterns.
Covers all modules с unit tests, integration tests, и performance benchmarks.

Test Structure:
- Unit tests для each module
- Integration tests для cross-module functionality
...
def generate_price_data(n_points: int = 1000, start_price: float = 100.0) -> np.ndarray:
    """Generate realistic price data для testing"""
    np.random.seed(42)  # For reproducible tests

    # Generate returns
    returns = np.random.normal(0.0005, 0.02, n_points)  # Daily returns с drift
...
def generate_ohlcv_data(n_points: int = 1000) -> pd.DataFrame:
    """Generate OHLCV data для testing"""
    np.random.seed(42)
    ...
    # Create DataFrame с datetime index
...
def generate_returns_data(n_points: int = 1000) -> np.ndarray:
    """Generate returns data для testing"""
    np.random.seed(42)

    # Mix of normal returns с some outliers
...
def assert_array_almost_equal(actual, expected, decimal=7):
    """Assert arrays are almost equal с proper handling of NaN"""
...
# Performance test thresholds (в milliseconds)
