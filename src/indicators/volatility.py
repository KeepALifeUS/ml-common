...
High-performance volatility indicators для crypto trading analysis.
Implements Context7 enterprise patterns с Numba optimization.

Available indicators:
- Standard Deviation
- True Range и Average True Range
- Keltner Channels
- Donchian Channels
...
def _fast_std(values: np.ndarray, period: int) -> float:
    """Fast standard deviation calculation с Numba"""
...
def _fast_true_range(high: np.ndarray, low: np.ndarray, close: np.ndarray) -> np.ndarray:
    """Fast True Range calculation для multiple periods"""
    if len(high) < 2:
        return np.array([0.0])
...
def calculate_true_range(
    ...
    # Manual calculation для last period
    if len(high) >= 2:
...
def calculate_keltner_channels(
    ...
    Returns:
        KeltnerChannelsResult с upper, middle, lower channels
    """
...
def calculate_donchian_channels(
    ...
    Returns:
        DonchianChannelsResult с upper, middle, lower channels
    """
...
def calculate_relative_volatility_index(
    ...
    # Calculate standard deviations для up и down moves
    std_devs = []
    directions = []
    ...
    # Separate volatilities for up и down days
    up_volatilities = []
    down_volatilities = []
...
class VolatilityConfig:
    """Configuration для volatility indicators"""
    std_period: int = 20
    atr_period: int = 14
...
class VolatilityIndicators:
    ...
    def calculate_all(
        ...
        Returns:
            Dictionary с all volatility indicator values
        """
        results = {}
