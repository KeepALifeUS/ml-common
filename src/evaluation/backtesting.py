...
High-performance strategy backtesting framework implementing Context7 patterns.
Optimized для crypto trading strategy validation с realistic trading conditions.

Features:
- Event-driven backtesting engine
- Position management с margin support
...
class Position:
    ...
    def update_market_value(self, current_price: float):
        """Update market value и unrealized PnL"""
...
class Portfolio:
    ...
    def update_positions(self, prices: Dict[str, float], timestamp: datetime):
        """Update all positions с current prices"""
...
def backtest_strategy(
    ...
    Args:
        strategy: Strategy object или signal generation function
        data: Market data с OHLCV columns
        config: Backtesting configuration

    Returns:
        BacktestResult с comprehensive performance analysis
    ...
    if data.empty or len(data) < 2:
        raise ValueError("Insufficient data для backtesting")
    ...
    try:
        ...
        for i, (timestamp, row) in enumerate(data.iterrows()):
            current_prices = {"main": row['close']}  # Simplified для single asset
...
def _execute_trade(
    ...
    """Execute trade с realistic costs"""
    try:
...
def _check_exit_conditions(
    ...
    """Check exit conditions для open positions"""
...
def _close_position(
    ...
    """Close position и update portfolio"""
    ...
    # Close all open trades для this symbol
...
def _analyze_drawdowns(equity_curve: np.ndarray) -> Dict[str, Any]:
    ...
    return {
        ...
        "drawdown_periods": int(np.sum(drawdowns > 0.01)),  # Periods с >1% drawdown
...
class BacktestEngine:
    ...
    Provides comprehensive backtesting framework с advanced features:
    - Multi-asset support
...
# Strategy wrapper для simple functions
class Strategy(BaseStrategy):
    """Simple strategy wrapper для function-based strategies"""
...
# Export all classes и functions
__all__ = [
    # Core classes
    "BacktestConfig",
    "BacktestResult",
