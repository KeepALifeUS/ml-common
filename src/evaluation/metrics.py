...
High-performance financial metrics calculation implementing Context7 patterns.
Optimized для trading strategy evaluation с proper risk-adjusted returns.
...
@dataclass
class MetricsConfig:
    """Configuration для performance metrics"""

    # Risk-free rate (annual)
    risk_free_rate: float = 0.02

    # Return calculation
    return_type: str = "simple"  # "simple" или "log"
    periods_per_year: int = 252  # Trading days per year

    # Drawdown settings
    drawdown_method: str = "peak_to_trough"  # "peak_to_trough" или "rolling_window"
    ...
    min_periods: int = 30  # Minimum periods для meaningful metrics
...
def _fast_drawdown(cumulative_returns: np.ndarray) -> Tuple[float, int, int]:
    """Fast maximum drawdown calculation с Numba"""
...
def _fast_var_historical(returns: np.ndarray, confidence: float) -> float:
    """Fast historical VaR calculation с Numba"""
    if len(returns) == 0:
        return 0.0
...
def calculate_cumulative_returns(returns: Union[np.ndarray, List[float]]) -> np.ndarray:
    ...
    # Cumulative product для simple returns
...
def calculate_sharpe_ratio(
    ...
    Args:
        ...
        periods_per_year: Periods per year для annualization
...
def calculate_max_drawdown(
    ...
    Args:
        returns: Returns series
        method: "peak_to_trough" или "rolling_window"
...
def calculate_var(
    ...
    Args:
        ...
        method: "historical", "parametric", или "monte_carlo"

    Returns:
        VaR value (positive для loss)

    Performance: ~0.2ms per 1000 data points с historical method
    """
    if len(returns) < 10:
        return 0.0
    ...
    if method == "historical":
        ...
        # Z-score для given confidence level
...
def calculate_beta(
    ...
    # Calculate covariance и variance
...
class PerformanceMetrics:
    ...
    Provides comprehensive performance analysis с proper statistical
    methods и Context7 enterprise patterns.
    """
    ...
    def calculate_all_metrics(
        ...
        Returns:
            PerformanceResult с all metrics
        """
    ...
    def compare_strategies(
        ...
        Args:
            strategy_returns: Dictionary с strategy names и returns

        Returns:
            DataFrame с comparison metrics
        """
        results = []
